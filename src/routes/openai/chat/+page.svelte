<script lang="ts">
    ...
    async function onSubmit(e) {
        ...
        const response = await fetch('/openai/chat', {
            method: 'POST',
            body: JSON.stringify({ question: data.question, history:history, context: data.context}),
            headers: {
...
<div class="container h-full mx-auto flex flex-col justify-center items-center">
        <h1 class="h1 p-5 my-5">Open AI - Chat</h1>
        <div class="p-2 bg-secondary-200 text-black">
            <p class="p-2 text-lg">In this example, we are showing how to have a chat with the Ai. The previous questions and answers are used as context for the next question </p>
            <p class="p-2 text-lg">This uses an API /openai/chat (POST) with the json request "chat": [chat array] and "context":"intended context of the chat"</p>
        ...
        <div class="m-10 p-10 bg-secondary-600 w-full text-center space-x-1">
            {#each chat as item}
                <div class="flex p-5 m-2 flex justify-center items-center {item.agent === "ai" ? 'bg-primary-700' : 'bg-secondary-700' }">
                    <div class="flex justify-center items-center mr-2 bg-white w-16 h-16 text-black rounded-full">{item.agent}</div> 
        ...
        <form class="w-full flex flex-col justify-center items-center" on:submit|preventDefault={onSubmit}>
            ...
            <div class="w-full flex my-5">
                ...
                <input class="p-5 flex-1 bg-secondary-100 text-secondary-700 text-lg" type="text" id="context" name="context" value="You are professor of mathematics" />
            </div>
